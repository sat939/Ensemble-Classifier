accuracies_for_various_runs(50)=0;
nn_accuracies_for_various_runs(50)=0;
knn_accuracies_for_various_runs(50)=0;
for round=1:1:50
clearvars -except accuracies_for_various_runs nn_accuracies_for_various_runs knn_accuracies_for_various_runs
clc
...
desired_accuracy=100%input('enter the desired accuracy of the training data classification so as to set the terminating condition\n 85 and above is preferable\n');
desired_epoch=100%input('enter the maximum epoch so as to set the terminating condition\n an epoch of around 125 is fine\n');
desired_max_delta_weight=0.000008%input('enter the desired_max_delta_weight so as to set the terminating condition\n some thing around 0.0008 must be fine\n');
...
for i=1:1:length(obtained_cluster_confidence_vector_temp)
    cl=find(obtained_cluster_confidence_vector_temp(i,:)==max(obtained_cluster_confidence_vector_temp(i,:)),1);
    test_cluster_obtained(i,1)=cl;
    
end
% 
% extra_col=zeros(size(obtained_cluster_confidence_vector_temp,1),size(obtained_cluster_confidence_vector_temp,2)+1);
% extra_col(:,1:size(obtained_cluster_confidence_vector_temp,2))=obtained_cluster_confidence_vector_temp;
% prob_correct=0;
% doub=[test_cluster_obtained test_data(:,size(test_data,2))];
% for i=1:1:size(doub,1)
% 
%     if doub(i,2)==doub(i,1)
%         prob_correct=prob_correct+1;
%         extra_col(i,size(obtained_cluster_confidence_vector_temp,2)+1)=1;
%     end
%  
% end
% extra_col
...
% 
% 
% nn_accuracies_for_various_runs(round,1)=probable_accuracy;
...
% extra_col1;
% probable_accuracy=prob_correct*100/size(doub,1)
% knn_accuracies_for_various_runs(round,1)=probable_accuracy;
% 
...
neural_network_fusion_training_input=cluster_confidence_matrix_temp;
neural_network_fusion_training_input(size(neural_network_fusion_training_input,1)+1:2*size(neural_network_fusion_training_input,1),:)=cluster_confidence_matrix_temp1;

% T=zeros(no_of_clusters,no_of_classes);
% clear sum
% for i=1:1:no_of_clusters
% for j=1:1:no_of_classes   
% T(i,j)=class_cluster_cooccurance_matrix(i,j)/(sum(class_cluster_cooccurance_matrix(i,:)));
% end
% end
%  
%  neural_network_fusion_training_output=zeros(length(training_data),no_of_classes);
% for i=1:1:no_of_classes
%     a=find(class_vector==i);
%     for j=1:1:length(a)
%     b=find(neural_network_fusion_training_input(a(j),:)==max(neural_network_fusion_training_input(a(j),:)));
%     neural_network_fusion_training_output(a(j),:)=T(b,:);
%     end
%     clear a;
% end
...
neural_network_fusion_training_output(size(neural_network_fusion_training_output,1)+1:2*size(neural_network_fusion_training_output,1),:)=neural_network_fusion_training_output;
...
f_no_of_units_in_each_layer(1,1)=no_of_clusters;
...
f_desired_accuracy=98%input('enter the desired accuracy of the training data classification so as to set the terminating condition\n 85 and above is preferable\n');
...
correct=0;
    ...
    clear chk;
    cvcv=class_vector;
    cvcv(size(cvcv,1)+1:2*size(cvcv,1),1)=class_vector;
    chk=[cvcv class_obtained];
for i=1:1:length(training_data)
    if chk(i,1)==chk(i,2)
        correct=correct+1;
    end
...
class_accuracy=100*correct/length(training_data)
    ...
    clear cond;
    cond=zeros(1,3);
    if class_accuracy>f_desired_accuracy
        cond(1,1)=1;
    end
...
neural_network_fusion_testing_input=obtained_cluster_confidence_vector_temp;
neural_network_fusion_testing_input(size(neural_network_fusion_testing_input,1)+1:2*size(neural_network_fusion_testing_input,1),:)=obtained_cluster_confidence_vector1_temp;
...
if find(obtained_class_confidence_vector_temp(occv,:)==max(obtained_class_confidence_vector_temp(occv,:)))==find(obtained_class_confidence_vector_temp(occv,:)==max(obtained_class_confidence_vector_temp(size(test_data,1)+occv,:)))
    obtained_class_confidence_vector(occv,:)=obtained_class_confidence_vector_temp(occv,:);
    ...
    if max(obtained_class_confidence_vector_temp(occv,:))>max(obtained_class_confidence_vector_temp(size(test_data,1)+occv,:))
    obtained_class_confidence_vector(occv,:)=obtained_class_confidence_vector_temp(occv,:);
    test_class_obtained(occv,1)=find(obtained_class_confidence_vector_temp(occv,:)==max(obtained_class_confidence_vector_temp(occv,:)));
    else 
    obtained_class_confidence_vector(occv,:)=obtained_class_confidence_vector_temp(size(test_data,1)+occv,:);
    test_class_obtained(occv,1)=find(obtained_class_confidence_vector_temp(size(test_data,1)+occv,:)==max(obtained_class_confidence_vector_temp(size(test_data,1)+occv,:)));
    end    
end
end
...
extra_col1=zeros(size(obtained_class_confidence_vector_temp,1),size(obtained_class_confidence_vector_temp,2)+1);
extra_col1(:,1:size(obtained_class_confidence_vector_temp,2))=obtained_class_confidence_vector_temp;
prob_correct1=0;
prob_correct2=0;
clear doub
for i=1:1:size(obtained_class_confidence_vector_temp,1)
zip(i,1)=find(obtained_class_confidence_vector_temp(i,:)==max(obtained_class_confidence_vector_temp(i,:)));
end
zap=test_data(:,size(test_data,2));
zap(size(zap,1)+1:2*size(zap),1)=test_data(:,size(test_data,2));
doub=[zip zap];
for i=1:1:size(doub,1)
if doub(i,1)==doub(i,2)
    if i<=size(doub,1)/2
        prob_correct1=prob_correct1+1;
    else
        prob_correct2=prob_correct2+1;
    end
   extra_col1(i,size(obtained_class_confidence_vector_temp,2)+1)=1;
end
end
extra_col1
probable_accuracy1=prob_correct1*100*2/size(doub,1);
probable_accuracy2=prob_correct2*100*2/size(doub,1);
nn_accuracies_for_various_runs(length(accuracies_for_various_runs)+1,1)=probable_accuracy1;
knn_accuracies_for_various_runs(length(accuracies_for_various_runs)+1,1)=probable_accuracy2;
...
obtained_class_confidence_vector
...
clear sum;
a=sum(accuracies_for_various_runs)/50;
b=sum(nn_accuracies_for_various_runs)/50;
c=sum(knn_accuracies_for_various_runs)/50;
[accuracies_for_various_runs nn_accuracies_for_various_runs knn_accuracies_for_various_runs]